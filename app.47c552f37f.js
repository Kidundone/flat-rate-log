...
function parseStockNumber(raw) {
  if (!raw) return { brand: "Unknown", type: "Unknown" };
  ...
  // Remove trailing A (trade reuse stock)
  const normalized = clean.replace(/A$/, "");

  // IMPORTANT: longest prefix first
  const rules = [...STOCK_PREFIX_RULES].sort(
