...
const WORK_PHOTO_BUCKET = "work_photos"; // <-- change if different

function normalizePhotoPath(p) {
  if (!p) return "";
  // If someone accidentally stored a full URL, strip it down to path after bucket
  try {
    if (p.startsWith("http")) {
      const u = new URL(p);
      // /storage/v1/object/public/<bucket>/<path>
      const idx = u.pathname.indexOf(`/object/public/${WORK_PHOTO_BUCKET}/`);
      if (idx >= 0) return u.pathname.slice(idx + `/object/public/${WORK_PHOTO_BUCKET}/`.length);
    }
  } catch {}
...
async function resolvePhotoUrl(photo_path) {
  ...
  const pub = sb.storage.from(WORK_PHOTO_BUCKET).getPublicUrl(path);
  const pubUrl = pub?.data?.publicUrl;

  // Quick probe: does it actually load?
  if (pubUrl) {
    try {
      const r = await fetch(pubUrl, { method: "HEAD" });
      if (r.ok) return pubUrl; // 200
    } catch {}
  }
...
async function openPhoto(entry) {
  ...
  meta.textContent = `Loading…  ${entry.photo_path}`;
  img.src = ""; // clear previous
  img.onerror = () => console.error("IMG_LOAD_FAIL", img.src, entry.photo_path);

  try {
    const url = await resolvePhotoUrl(entry.photo_path);
    meta.textContent = entry.photo_path;
    img.src = url + `&cb=${Date.now()}`; // cache-bust for iOS/SW weirdness
    document.getElementById("photoModal").classList.add("open");
  } catch (e) {
    console.error("PHOTO_LOAD_ERROR", e, entry.photo_path);
...
async function entryPhotoUrl(entry) {
  ...
  // If Supabase storage path exists, resolve URL
  if (entry?.photo_path) {
    try { return await getPhotoUrl(entry.photo_path); } catch {}
  }
  return null;
}
