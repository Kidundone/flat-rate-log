...
function openPhoto(entry) {
  ...
  const row = entry;
  const SUPABASE_PROJECT = "lfnydhidbwfyfjafazdy";
  const BUCKET = "work_photos";

  const fullUrl =
    `https://${SUPABASE_PROJECT}.supabase.co/storage/v1/object/public/` +
    `${BUCKET}/` +
    row.photo_path;
  ...
  img.onerror = () => {
    ...
    toast("Photo missing or was not uploaded");
  };

  img.src = fullUrl;

  meta.textContent = row.photo_path || "";
