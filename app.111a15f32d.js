...
async function openPhoto(row) {
  ...
  if (!path) return toast("No photo for this entry");

  const { data, error } = await window.sb.storage.from(bucket).createSignedUrl(path, 600);
  if (error || !data?.signedUrl) {
    console.log("signedUrl error", error);
    return toast("Photo failed to load");
  }
